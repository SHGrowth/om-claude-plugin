...
### Empirical tuning — three designs rejected against the real ESA data before this shipped

The plan's draft algorithm was implemented literally first, then measured against the full ESA run (89 done stories × 229 snapshot candidates) — three of its ideas failed live and were replaced, each verified against the binding case (Story 3.1's manual fix reverted, expecting the report to flag it with PR #29):
...
- **The proper-noun gate's stated premise was false on its own validation data.** "OM PR titles follow conventional-commit lowercase" — the real ESA snapshot holds at least 7 title-case PR titles ("Fix customer account invitation emails", "Show deal created date…"), whose capitalized ordinary first words minted junk single-stem flags. The gate now requires a capital/digit *beyond the first character* (`KSeF`, `DataTable` qualify; `Fix`, `Show` — and single-capital product names like `Magento`, which fall back to the multi-stem path — do not).
