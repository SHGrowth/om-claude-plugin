...
## 1.23.0 — gap-analysis batch mode: Upstream-pipeline citation cross-check

A fifth structural gate for the batch mode, alongside the verdict gate (`gap-validate-finding`, I019/I038), the intake gate (`gap-checklist-gate`, I024), and the two Phase-2 preflights (`gap-orientation-preflight` I037/I038, `gap-grounding-preflight` I036/I038). All four existing gates are **untouched**. Source plan: a client engagement's process-improvement proposal (2026-07-08, kept with the engagement materials, not in this repo) — the first gate sourced from a live engagement's observed failure rather than an agents-master improvement spec.

### The hole

The **Upstream pipeline** field's *content* was the only per-story output decided purely by each subagent's isolated judgment against a one-line PR title — `bin/gap-validate-finding` checks the field's *shape* only, by design (it is non-verdict-altering, so it was never given the Verdict/Evidence anti-fabrication rigor). Observed on the real engagement run (2026-07-08, 94 stories): a manual-KSeF-e-invoice-export story (Story 3.1) failed to cite `official-modules PR #29` ("feat: financial + full KSeF feature set integration") even though both titles contain "KSeF" verbatim, while two sibling stories in the same run cited it correctly. 18 re-trials of the identical prompt in isolation never reproduced the miss — so this is rare per-call variance, and multiplied across ~94 independent calls per run it will keep producing a handful of misses no matter how good the prompt is. The fix is structural and orchestrator-side (the same principle that grounds verdicts), not a "try harder" prompt rewrite — asking an LLM to self-police a judgment it already got wrong once is the documented S012 anti-pattern.

### Added — `bin/gap-pipeline-crosscheck`, the Phase 2→3 citation cross-check

Runs once per batch, after Phase 2's per-story loop and before Phase 3 synthesis. Deterministic, no model in the loop: tokenizes every `done` story's title + acceptance criteria and every snapshot PR/spec title with the S012 stemming/stopword logic `gap-validate-finding` already uses (one heuristic across both checks, plus one *declared* addition — an English function-word stoplist, because story prose unlike grounding queries is full of 4+-letter function words). A (story, candidate) pair clears the match threshold on ≥2 shared scoring stems, or on a single shared stem that is unique to that one candidate in the whole snapshot *and* carries a capital or digit beyond its first character in the original title (`KSeF`, `DataTable` — an internally-cased/numbered token is an acronym or product name, exactly the single-token matches worth surfacing; a first-letter capital alone deliberately does not count, because real snapshot titles include title-case ordinary words like "Fix customer account invitation emails"). A story whose citation *resolves* to a real snapshot entry is satisfied; it is flagged in exactly two cases — it cites nothing while above-threshold candidates exist (a missing citation), or it cites a well-formed value found nowhere in the snapshot (a phantom citation, flagged for every done story regardless of candidates, since subagents read only this snapshot). The report (stdout) gives each flagged story's currently-cited value (phantoms explicitly labeled) and the full IDF-ranked candidate list annotated with the shared stems — the orchestrator/human reviews it and hand-edits only real misses; dismissing a coincidental overlap is a legitimate outcome. Never an auto-fix; only ever touches the **Upstream pipeline** field; structurally incapable of altering Verdict/Evidence/Effort. `needs-review` stories are skipped (their placeholder was never legitimately populated). Wrong input files fail closed (exit 2), mirroring the sibling gates. Scope honesty: this closes the *consistency* gap (94 isolated guesses vs one deterministic pass over the same data), not the *semantic-relevance* gap — lexical flags can false-positive, and synonym-phrased true matches can still be missed.

### Empirical tuning — three designs rejected against the real engagement data before this shipped

The plan's draft algorithm was implemented literally first, then measured against the full engagement run (89 done stories × 229 snapshot candidates) — three of its ideas failed live and were replaced, each verified against the binding case (Story 3.1's manual fix reverted, expecting the report to flag it with PR #29):

- **Snapshot-side-only distinctiveness (the plan's draft) flagged 74/89 stories.** Generic dev vocabulary (`status`, `email`, `create`, `field`) sits under any within-snapshot frequency cutoff while saturating story text. Replaced by the centrality test (a stem scores only when in the story's title or occurring 2+ times in its text) plus the unique-stem/proper-noun path.
- **A story-side rarity filter silently killed the true positive.** The obvious symmetric fix (a stem in >5 done stories never scores) removed `ksef` itself — an engagement *about* KSeF mentions it in more stories than any within-run cutoff allows. The binding case failed; the filter was removed entirely. What mechanically separates `ksef` from generic single-token collisions (`upload`, `export`, `client`) is snapshot-side uniqueness plus the proper-noun property, not story-side rarity.
- **The plan's literal satisfaction rule ("cited must be above-threshold") false-flagged all three stories correctly citing PR #29** — including the two the plan itself names as known-correct baselines (2.14, 4.10) — whenever the lexical scorer under-scored the true match. Replaced by: citing any real snapshot entry satisfies; the failure mode under attack is a *missing* citation, and second-guessing *which* entry a subagent chose is semantic work a lexical falsifier has no standing to do.

Final defaults on the real run (binding configuration, Story 3.1's fix reverted): 37 flagged stories, ~2 candidates each (146 report lines), with the binding case reporting exactly `official-modules PR #29` as Story 3.1's sole candidate — reviewable in one pass, against 74 flags and a 3,465-line report (~45 candidates per flag) under the draft algorithm.

### Changed — `skills/om-cto/references/gap-analysis-batch.md`

Phase 2 gains step 7 (run the cross-check at the Phase 2→3 transition, review the report, hand-fix or dismiss each flag; the old step 7 is now step 8 and requires the report to have been reviewed), the residual-failure paragraph gains the citation-miss case, the Cross-refs table gains the new gate, and acceptance test 13 records the fixture + binding-case behavior.

### Review — seventeen issues caught and fixed before this branch shipped

A five-lens adversarial review (awk correctness, plan fidelity, parsing robustness, doc consistency, design attack) with per-finding refutation verification confirmed 15 issues against the first complete implementation; an independent second-pass review of the fixed result found 2 more. All fixed and re-verified:

- **Phantom citations went unchecked when a story had no lexical candidates (the one real hole).** The no-candidates early-out ran before citation parsing, so a fabricated but well-formed citation (`official-modules PR #999 (open)`) on a lexically-quiet story passed clean — contradicting the header's own satisfaction wording. Fixed mechanically, not just documented: citation existence against the snapshot is now checked for *every* done story, and a value resolving to no snapshot entry is flagged as a labeled phantom regardless of candidates. Zero false-positive risk — subagents and the cross-check read the same static snapshot file.
- **CRLF inputs failed OPEN.** A CRLF MD parsed its story headings (prefix-anchored) but never matched `Status: done` (`done\r`), producing a normal-looking "0 done stories checked" PASS — the exact silent-vacuous-pass class this gate family exists to prevent; a CRLF snapshot silently dropped every `$`-anchored planned-spec line. Fixed by stripping `\r` once at the top of the awk program (plus `\r` in trim's trailing class).
- **macOS awk fatal on Polish diacritics.** `substr(w, 1, 5)` is byte-oriented; a multibyte character straddling byte 5 (`Kraków` → `krak\xC3`) produced an invalid UTF-8 stem that crashed awk 20200816 the moment `sub()`/`gsub()` touched it — a truncated report on exactly the Polish-language engagements this mode serves. Fixed with `LC_ALL=C` (byte-oriented end to end; both sides tokenize identically).
- **The proper-noun gate's stated premise was false on its own validation data.** "OM PR titles follow conventional-commit lowercase" — the real engagement snapshot holds at least 7 title-case PR titles ("Fix customer account invitation emails", "Show deal created date…"), whose capitalized ordinary first words minted junk single-stem flags. The gate now requires a capital/digit *beyond the first character* (`KSeF`, `DataTable` qualify; `Fix`, `Show` — and single-capital product names like `Magento`, which fall back to the multi-stem path — do not).
- **`--max-title-freq 0` was accepted and silently disabled the whole check** (cutoff 0 < every df ≥ 1 → unconditional PASS), and the `--max-title-freq=` empty-value form silently fell back to the default while `--min-distinctive=` errored. Both now exit 2.
- **The ranking comment promised a stability the selection sort didn't have** — tied candidates could swap out of snapshot order. Replaced with a stable insertion sort.
- **Stale/wrong documentation, five findings:** the exit-code contract and the skill doc's exit-0 bullet still stated the abandoned draft satisfaction rule; the header's single-stem description omitted the proper-noun gate the code enforces; the CHANGELOG's own "second above-threshold candidate" fixture claim was untrue (fixed by adding a real second candidate, PR #109, which now outscores PR #29 — making the one-true-match-satisfies property genuinely exercised rather than vacuously claimed); the header's "63/89 flagged without the centrality test" figure didn't reproduce (re-measured: 65/89 on the binding configuration); and the draft-algorithm noise figure "~19 candidates per flag" understated the measured report (74 flags, 3,465 lines, ~45 candidates per flag).
- **(second pass) Spec phantoms could falsely resolve by substring.** Citation resolution for specs used substring containment both directions, so a cited path that merely *contains* a real snapshot filename (`…tenant-webhooks-outbox.md.bak`) passed as real — hollowing out the phantom guarantee for exactly the spec kind. Now exact basename equality, with a missing `.md` extension as the one tolerated variation; verified to introduce zero false phantoms on the real engagement run (37/36 flags unchanged).
- **(second pass) This entry's own edit had clobbered the `## 1.22.0` heading**, leaving the 1.22.0 release notes headingless under 1.23.0. Restored.

### Verification

Fixtures (`docs/specs/fixtures/gap-pipeline-crosscheck/`): `flagged.md` → exit 1 with two flags — Story 3.1 (cites `none`, ranked candidates: PR #109 [manua, expor] above `official-modules PR #29` [ksef]) and Story 5.2 (cites `official-modules PR #77 (open)`, labeled a phantom — not in the snapshot); `satisfied.md` (3.1 citing PR #29, 5.2 citing the real `PR #108`) → exit 0 — 3.1 passes despite the *higher-scored* candidate PR #109 remaining uncited (one true match satisfies), and 5.2 documents the accepted limit that any resolving citation satisfies. Binding case verified against the *real* engagement artifacts: with Story 3.1's citation reverted to `none` in a scratch copy, the report flags 3.1 with exactly `official-modules PR #29 (open)` [shared stem: ksef]; with the shipped MD (3.1's fix in place), 3.1 does not flag (37 vs 36 flags, delta exactly 3.1). `needs-review` fixture story sharing `barcode` with two candidates → skipped (4 done checked, not 5). `invoice` (7 of 13 fixture titles) → non-distinctive, no match alone; `number` (snapshot-unique but lowercase) → excluded by the proper-noun gate. CRLF copies of both fixtures → identical results to LF. Diacritic-bearing story/title (`Kraków`) → matches cleanly, no crash. A spec citation whose path merely contains a real snapshot filename (`…outbox.md.bak`) → flagged as a phantom, with the real spec ranked as its candidate; the same citation missing only its `.md` extension → resolves. Tied candidates → reported in snapshot order. No args / missing file / swapped inputs / non-integer, zero, or empty flag values → exit 2 with the concrete complaint. Empty-but-well-formed snapshot → exit 0, "0 candidates". Runs in 0.14 s on the 419 KB engagement MD under macOS BSD awk (20200816) and bash 3.2-compatible constructs.

## 1.22.0 — gap-analysis batch mode: local-first grounding — develop, local grep, PR/official-modules/specs (I038)

Collapses the three evidence sources batch mode had accumulated by v1.21.0 (vendored `om-reference/`, live `gh search code`, I037's orientation-only checkout) down to two validated local checkouts, and widens Phase 2 beyond merged code on one repo/branch. Source spec: `agents-master/improvements/I038.md`. Unlike I036/I037, this release *does* touch the existing gates — it re-scopes I019's grounding mechanism and I036's channel preflight, and closes an I019 §88 residual hole outright; `bin/gap-checklist-gate` (I024) is the only sibling gate untouched.

### The hole

`main` undercounts OM's actual codebase by a release cycle on **both** repos: verified live, `origin/develop` sits **980 commits ahead** of `main`'s last release cut on `open-mercato/open-mercato` (`v0.6.5`) — real feature and security work (`feat(customers): add deals map view tab`, several tracked `security(...)` hardening commits) — and **16 commits ahead** on `official-modules`. A `main`-only checkout or `gh search code` (which indexes the default branch) simply cannot see this. Separately, once a checkout exists and is kept fresh, re-querying GitHub's rate-limited search API for code already sitting verified on disk is redundant — it only keeps alive the sequential-drain discipline I019 built specifically to survive that API's rate limit, and that same discipline shape-trusted a `live`-sourced `✅`/`🟡` (never re-running it) purely to protect that budget — a rationing with nothing left to justify once grounding is a free local search. And even a perfect `develop` checkout of the core repo only shows *merged core* code — it says nothing about a capability shipped as a separate `official-modules` package, mid-review as an open PR, or already speced in `.ai/specs/` but not yet built.

### Changed — `bin/gap-orientation-preflight`: validates TWO checkouts, `develop`, hard-enforced freshness, reuse-then-provision

`DEFAULT_BRANCH` moves from `main` to `develop`, for **both** `open-mercato/open-mercato` (required — hard-fails the preflight) and the new second checkout, `open-mercato/official-modules` (best-effort — a smaller auxiliary repo being briefly unreachable degrades gracefully to `UNAVAILABLE` on stdout line 2 rather than blocking the whole run). Because these checkouts now carry verdict grounding, not just orientation, staleness can no longer be I037's soft note — the preflight `git fetch`s and fast-forwards for real, then asserts `HEAD` actually equals the remote tip (a plain `merge --ff-only` is a silent no-op when already ahead, so it alone can't catch a checkout carrying local-only commits), and refuses to touch a checkout with uncommitted changes. Remote resolution captures whichever remote's URL actually matches upstream and uses *that* name for every fetch/merge, rather than assuming `origin` — a checkout with `origin=<fork>` and a separately-named upstream remote (a common contribution layout) would otherwise ground silently against the fork. Resolution order is now: an explicit path override → the first candidate in a short, conservative list that *already* validates (core defaults to `~/Documents/open-mercato`, never mutated if it doesn't already qualify) → this script's own dedicated, reused cache checkout, cloned once. The HEALTHY line also reports how many commits `develop` sits ahead of `main` per repo, a run-level (not per-finding) honesty note about unreleased-work risk. Since it now performs real network operations, it gained a genuine **exit 2** (transient network failure) that I037's pure-validator version didn't need.

### Changed — `bin/gap-validate-finding`: local `git grep`/`rg` replaces live `gh search code`; hole 3 closed

Verdict grounding is now a local content-*or*-path search (a query can legitimately be a repo-relative path like `packages/<official-module-name>`, which a content-only `grep`/`rg` structurally cannot see) against `--repo-root` or `--official-modules-root`, whichever `Grounding source` names — same falsifier logic, same S012 strawman guard (now applied uniformly to every verdict, not just `❌`), different backend. **Every grounded verdict is re-run unconditionally, no exceptions** — I019's shape-trust exemption for a `live`-sourced positive existed only to ration GitHub's rate-limited API; a local search has no such cost, so I038 closes that residual hole (I019 §88 hole 3) instead of carrying it forward. `Grounding source` is `checkout` | `official-modules` — no `vendored`: I038 dropped the separately-vendored copy this mode used to fall back to, so a third value that behaved identically to `checkout` (since every source is re-run now) would only invite a "didn't really check" framing; an unrecognized or `vendored` value is rejected outright. **Exit 2 (RETRY-LATER) is retired** — a local search has no rate limit and no transient-network failure mode. New required field **Upstream pipeline** (`none` | `PR #<n> (open)` | `official-modules PR #<n> (open)` | `spec: <path> (planned, unbuilt)`) — shape-checked only, never re-run, and structurally incapable of upgrading a `✅`/`🟡`/`❌` verdict.
...
Preconditions, Step 0, the subagent template, the source-of-evidence table, and the acceptance tests all updated to match the dual-checkout, always-re-run design — including a real `packages/<official-module-name>` capability now counting as genuine `✅`/`🟡` coverage, not merely the supplementary Upstream-pipeline PR signal. Added a **one-time, never-per-story** `gh` fetch of open PRs (both repos) + planned specs, written to a snapshot file every subagent reads instead of calling `gh` itself — this closes a real gap the I038 spec's own "assumptions to verify" list had left open: dispatching all pending subagents in one message (per I023) while each independently called `gh pr list` would have reintroduced the exact parallel-rate-limit problem I019 solved for a different channel. Caught and fixed before any code was written, not after.

### Review — seven issues caught and fixed before this branch shipped

Four review rounds against the diff (not the design) found six real, independently-reproduced issues; a seventh (#7) was self-caught while fact-checking this very entry's numbers against live data before writing them down. All seven fixed and re-verified before landing:

- **#1 wrong-remote fetch (P1).** The remote check accepted *any* remote matching `open-mercato/open-mercato`, but the fetch/merge was hardcoded to `origin` — so a checkout with `origin=<fork>` and a *different* remote (e.g. `upstream`) pointing at the real repo passed validation while silently grounding against the fork. Reproduced against a realistic fork-with-shared-history layout, not just a synthetic one: preflight reported HEALTHY while checkout content stayed the fork's. Fix: capture the matched remote's name and use it for every subsequent `fetch`/`merge`/error message.
...
- **#4 shape-trust not actually closed (P1).** The first implementation pass still shape-trusted a `checkout`-sourced `✅`/`🟡` — unchanged from I019's pre-I038 exemption — even though the entire rationale for that exemption (rationing a rate-limited API) no longer applied to a free local search. Reproduced: a fabricated `checkout`-sourced `✅` with zero real hits passed clean. Fix: every grounded verdict is now re-run, full stop; `Grounding source` is provenance only.
- **#5 official-modules was PR-signal only, not real evidence (P1, the main gap).** The reference doc's Phase-2 design only ever snapshotted official-modules' *open PRs* — never its actual shipped code — so a capability already merged there as a real module would incorrectly read as `❌ Missing`. Fix: `bin/gap-orientation-preflight` validates a second checkout (`official-modules@develop`), `bin/gap-validate-finding` gained `--official-modules-root` and grounds `official-modules`-sourced claims against it. Verified against a real shipped package (`packages/carrier-inpost`).
- **#6 content-only search rejected path-shaped queries (P1).** The subagent prompt explicitly recommends path-shaped grounding queries (`modules/<x>`, `packages/<official-module-name>`) but `local_search()` only ever ran content search (`rg`/`git grep`), which cannot see a path that isn't echoed as text anywhere. Reproduced: `packages/carrier-inpost/src/modules/carrier_inpost` is a real, existing directory with zero content hits — a subagent following the documented guidance would have its correct `✅` wrongly rejected. Fix: fall back to a path check (`git ls-files | grep -i`) whenever content search cleanly returns "no match," never masking a real search error.
- **#7 the unreleased-work honesty note was silently wrong (P2, self-caught while fact-checking this entry).** The "`develop` is N commits ahead of `main`" note computed `N` via `git rev-list --count` on the `--depth 1` cache checkout after fetching `main` — but a shallow clone doesn't carry enough history for that count to be correct even after the fetch. Verified against GitHub's own compare API: the script reported 1 commit for `official-modules` (true value: 16) and 920 for the core repo (true value: 943) — both confidently wrong, in the wrong direction (undercounting the very thing the note exists to surface honestly). Fix: compute `N` via `gh api repos/<repo>/compare/main...develop --jq .ahead_by` instead of local git — full history regardless of local shallow depth, and simpler. Best-effort: skipped silently if `gh` is unavailable, same as before.

### Verification

Verified live throughout, including realistic fork-vs-upstream repros (shared history, fork stale + carrying its own WIP commit, upstream ahead) rather than synthetic-only setups: auto-managed clone, reuse of an already-qualifying candidate checkout (and correctly skipping one that doesn't, e.g. a real `~/Documents/open-mercato` sitting on `main`), and best-effort degradation when official-modules is unreachable; fork-on-feature-branch refusal (the I037 binding case, now against `develop`, for both checkouts); dirty-working-tree refusal; staleness fast-forward (seed a checkout behind its remote, confirm the preflight catches it up and asserts equality); the wrong-remote-fetch fix; the ahead-of-remote fix; the shape-trust closure (a fabricated `checkout`-sourced positive now fails; a genuine one still passes); official-modules grounding against real shipped content, a fabricated official-modules claim, and a missing-root rejection; the path-vs-content search fix; the TagsInput-trap, empty-positive, and S012 strawman regressions; the Upstream-pipeline shape check across all four recognized values plus the single-digit-PR and junk-digit fixes; a rejected `vendored` `Grounding source`; missing `--repo-root`/`--official-modules-root`; missing Upstream-pipeline field; `gap-grounding-preflight`'s gh-missing and repo-unreachable failures; the unreleased-work honesty note against GitHub's own compare API for both repos (943 and 16 commits respectively, matching exactly) and its silent, no-fail degradation when `gh` is unavailable. `bash -n`, `git diff --check`, and `scripts/check-version-sync.sh` all pass against the current (pre-manifest-bump) state.
